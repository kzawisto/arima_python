...
def choose_arma_model(data, max_ar_lags=3, max_ma_lags=3, criterion='bic'):
    ...
    return results, sorted([r for r in results.values() if not np.isnan(r['criterion'])], key=lambda x: x['criterion'])[
        0]


def choose_arima_model(data, max_ar_lags=3, max_ma_lags=3, max_i=0, criterion='bic'):
    results = {}
    if isinstance(criterion, str):
        criterion = {
            'bic': bic, 'aic': aic
